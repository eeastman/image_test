...
def pca(batch_size):
    ...
    #X = np.zeros((100, 5, 6, 7)) # 100 images suppose  # REPLACE WITH SAVED OUTPUT FROM LAYER
    X = np.load('./pool_layers/pool5.npy')
    X = X.reshape((batch_size, -1)) # REPLACE 100 W/ NUMBER OF IMAGES 
...
def run_svm(batch_size):
    X_imgs = np.load('./pool_layers/pool5.npy')
    X_imgs = X_imgs.reshape((batch_size, -1))
    y_labs = np.array([1, 1, 0, 0]) # NEED LABELS
